...
public class PathGenerator {
     ...
     *
     * <p>
     * The returned path contains data which can be passed to motion profile to
     * execute the path. The robot will accelerate at a constant rate, then reach a
     * constant cruise velocity, then decelerate at the same rate. In other words,
     * the velocity vs time graph shows an isosceles trapezoid.
     *
     * @param waypoints
     *                               points that the path should pass through
     * @param cruiseVelocity
     *                               the target constant cruise velocity of the
     *                               robot in in/100ms
     * @param targetAcceleration
     *                               the target constant acceleration (and
     *                               deceleration) of the robot in in/100ms/s
     * @return an array of points on the path
     */
    public static PathPoint[] generatePath(Point[] waypoints, double cruiseVelocity, double targetAcceleration) {
    ...
    public static QueryData getQueryData(Point[] waypoints) {
        ...
        queryData.totalWaypointDistance = queryData.cumulativeWaypointDistances[queryData.cumulativeWaypointDistances.length
                - 1];
        queryData.queryCount = (int) Math.ceil(queryData.totalWaypointDistance / QUERY_INTERVAL) + 1;
        return queryData;
    }
    ...
    private static double[] calculateCumulativeDistances(Point[] waypoints) {
        ...
        for (var i = 1; i < waypoints.length; i++) {
            cumulativeDistances[i] = cumulativeDistances[i - 1]
                    + Math.hypot(waypoints[i].x - waypoints[i - 1].x, waypoints[i].y - waypoints[i - 1].y);
        }
    ...
    private static void calculateAngles(double[] dx, double[] dy, PathPoint[] path) {
        ...
        // of knowing
        // the actual angle of the robot), we estimate the real unwrapped angle by
        // looking at if the
        // current angle would be closer to the previous angle after adding or
        // subtracting 360. This
        // is fine because the robot can't turn more than 180 degrees in between two
        // data points on
    ...
    public static void calculatePathPoints(PathPoint[] path, double cruiseVelocity, double targetAcceleration,
            SplinePair splines, QueryData queryData) {
        ...
        // Arclength
        // formula integrate(hypot(dy/dt, dx/dt)*dt) where dt is QUERY_INTERVAL.
        // Rectangles are
        // centered at each querypoint, so the cumulative area under curve is half a
        // rectangle each
        // from the last point and the current point. The interval for the last
        // pathpoint is shorter
        // as it is the last waypoint, and QUERY_INTERVAL does not divide into the full
        // path length.
        for (var i = 1; i < path.length; i++) {
            double interval = i == path.length - 1 ? queryData.totalWaypointDistance % QUERY_INTERVAL : QUERY_INTERVAL;
            path[i].position = 0.5 * interval
                    * (Math.hypot(dxQueries[i], dyQueries[i]) + Math.hypot(dxQueries[i - 1], dyQueries[i - 1]))
                    + path[i - 1].position;
        }
        var totalDistance = path[path.length - 1].position;

        // Calculations for velocity and time are in three separate stages for the
        // acceleration,
        // constant velocity, and deceleration parts respectively.
        {
            var reachesCruiseVelocity = false;
            // First stage: we find velocity and time for data points assuming constant
            // acceleration
            // up until either (1) we reach the cruise velocity, or (2) we have not reached
            // cruise
            // velocity and we are halfway through the path already, in which case we must
            // start
            // decelerating.
            int i;
            ...
            // Third stage for velocity: we do the same thing as the first stage, but
            // working
            // backwards instead. We stop when either (1) we reach the cruise velocity, or
            // (2) we
            // reach where stage 1 stopped, in which case there is no constant velocity
            // part.
            int j;
            for (j = path.length - 1; j >= i; j--) {
                var velocity = Math.sqrt(2 * targetAcceleration * (totalDistance - path[j].position));
                if (velocity > cruiseVelocity) {
                    break;
                }
            ...
            int k;
            // Second stage: we calculate velocity and time in between the end of first
            // stage and
            // start of third stage. If we never reach the cruise velocity, this loop never
            // runs.
            for (k = i; k <= j; k++) {
                ...
                // delta_x = x - x0
                // t = t0 + (x - x0) / v
                path[k].time = cruiseStartTime + (path[k].position - cruiseStartPosition) / cruiseVelocity;
            }
            var decelerateStartPosition = reachesCruiseVelocity
                    // It takes the same distance to accelerate and decelerate to/from the same
                    // velocity
            ...
            var decelerateInitialVelocity = reachesCruiseVelocity ? cruiseVelocity
                    // v^2 = 2 a x_mid
                    // v = sqrt(2 a x_mid)
                    // x_mid = x_total / 2
                    // v = sqrt(a x_total)
